...
export async function generateMetadata(props: PageProps<"/term/[slug]">): Promise<Metadata> {
  const { slug } = await props.params;
  const term = getTerm(slug);
  if (!term) return { title: "Term not found" };

  const title = `${term.term} - meaning, origin and usage`;
  return {
    title,
...
export default async function TermPage(props: PageProps<"/term/[slug]">) {
  const { slug } = await props.params;
  const term = getTerm(slug);
  if (!term) notFound();
        ...
        <Container className="py-12 sm:py-16">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="font-mono text-[11px] tracking-widest text-ink-subtle uppercase">
              {statusCopy[term.status]}
            </span>
          </div>

          <h1 className="mt-6 font-display text-6xl leading-[0.9] tracking-tight sm:text-8xl">
            {term.term}
          </h1>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          ...
          <div className="card mt-4 p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {term.tags.map((tag) => (
                <Link key={tag} href={`/tag/${encodeURIComponent(tag)}`} className="focus-ring rounded-full">
                  <Badge variant="outline" size="md" className="transition hover:border-line-strong hover:text-ink">
                    {tag}
                  </Badge>
                </Link>
              ))}
